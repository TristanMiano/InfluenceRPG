...
-- Table for Game State Summaries
CREATE TABLE IF NOT EXISTS game_history (
    id             SERIAL PRIMARY KEY,
    game_id        UUID       NOT NULL,
    summary        TEXT       NOT NULL,
    summary_date   TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    embedding      VECTOR(384) NOT NULL,      -- adjust dim to your SentenceTransformer model
...
-- And index on the embedding vector for similarity search:
CREATE INDEX IF NOT EXISTS idx_game_history_embedding
  ON game_history
  USING ivfflat (embedding vector_cosine_ops)
  WITH (lists = 100);

-- Analyze so the index can build
VACUUM ANALYZE game_history;
